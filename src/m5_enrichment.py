...
def enrich_chunks(
    ...
    if methods is None:
        methods = ["contextual", "metadata"] # default to cheaper ones for now

    enriched = []
    # limit to first few chunks to save time and tokens during lab
    subset = chunks[:10] if len(chunks) > 10 else chunks
    
    print(f"  Enriching {len(subset)} chunks...")
    for chunk in subset:
        ...
        if "contextual" in methods or "full" in methods:
            enriched_text = contextual_prepend(chunk["text"], chunk["metadata"].get("source", "Tài liệu"))
